...
def train_load(path, parts):
    ...
    return features, speakers, nspeakers # [N, seq_len, 64]
...
def dev_load(path):
    ...
    # #revision by Wenting
    # x=np.concatenate([enrol,test])
    # trials=data['trials']
    # trials[:,1]=trials[:,1]+enrol.shape[0]

    return data['trials'], data['labels'], enrol, test  # [M, 2] numpy array, [M]
    # return trials, data['labels'], x
